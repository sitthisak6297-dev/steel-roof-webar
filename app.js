...
async function start(){
 ...
 model=gltf.scene;roof=null;gates=[];
  
 scene.add(model);bounds=new THREE.Box3().setFromObject(model);bounds.max.z=Math.max(bounds.max.z,3.85);state.ready=true;$('loading').hidden=true;for(const id of ['gates','roof','reset','ar'])$(id).disabled=false;resetView(false);announce('ประตูปิด · ใส่หลังคา');
